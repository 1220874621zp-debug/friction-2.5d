...
// Puppet-pin skin deformation core: a triangle mesh over the image
// driven by additive pin offsets (bones drive PINS, pins drive the
// mesh - there is no direct bone/mesh path anymore).
class BoundingBox;
...
// all skin state of one image layer
struct SkinBindData {
    SkinMesh fMesh;
    QTransform fBindBoxTotal;   // image rel -> scene at mesh creation
};
...
// (0 = tight grip with no hold, 1 = widest/softest linear spread);
// 0.5 is the validated default (20% hold + cubic falloff)
CORE_EXPORT float pinWeight(const qreal dist, const qreal radius,
                            const qreal softness = 0.5);

// offline self-test (mesh generation, pin falloff/tracking AND the
// drawVertices render call path used by ImageRenderData::drawSk);
