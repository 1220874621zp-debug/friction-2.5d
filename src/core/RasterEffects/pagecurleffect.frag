...
vec4 sampleTex(const vec2 uv) {
    ...
    const vec2 base = floor(t);
    const vec2 f = t - base;
    const vec2 st = vec2(1.0) / uTexSize;
    const vec4 c0 = texture(tex, (base + vec2(0.5, 0.5)) * st);
    const vec4 c1 = texture(tex, (base + vec2(1.5, 0.5)) * st);
    const vec4 c2 = texture(tex, (base + vec2(0.5, 1.5)) * st);
    const vec4 c3 = texture(tex, (base + vec2(1.5, 1.5)) * st);
    return mix(mix(c0, c1, f.x), mix(c2, c3, f.x), f.y);
}

void main(void) {
    const float aspect = uTexSize.x / uTexSize.y;
    // pixel space scaled to units of image height (square pixels)
    const vec2 p = vec2(texCoord.x * aspect, texCoord.y);

    const float radDir = radians(uDirection);
    const vec2 dir = vec2(cos(radDir), sin(radDir));
    const vec2 perp = vec2(-dir.y, dir.x);

    // how far the contact line can travel: page extent along dir
    const float cMax = max(max(dot(vec2(0.0, 0.0), dir),
                               dot(vec2(aspect, 0.0), dir)),
                           max(dot(vec2(0.0, 1.0), dir),
                               dot(vec2(aspect, 1.0), dir)));
    const float c = (1.0 - uProgress) * cMax;
    const float R = max(uRadius, 0.002);

    const float d = dot(p, dir) - c;      // signed distance from the contact line
    const float m = dot(p, perp);         // position along the curl axis

    // the wrapped page lands at -R*sin(phi): solve for every turn and
    // keep the frontmost (largest height R*(1-cos(phi)))
    float bestZ = -1.0;
    float bestPhi = -1.0;
    const float q = clamp(-d / R, -1.0, 1.0);
    const float phi0 = asin(q);
    const float phiCap = 11.5 * PI;       // ~5.75 turns is plenty
    for (int k = 0; k < 8; k++) {
        const float base = TWO_PI * float(k);
        const float f1 = phi0 + base;
        if (f1 > phiCap) break;
        if (f1 >= 0.0) {
            const float z = R * (1.0 - cos(f1));
            if (z > bestZ) { bestZ = z; bestPhi = f1; }
        }
        const float f2 = PI - phi0 + base;
        if (f2 > phiCap) break;
        if (f2 >= 0.0) {
            const float z = R * (1.0 - cos(f2));
    ...
    vec2 uvSrc = texCoord;
    if (useWrap) {
        const vec2 P0 = dir * (c + R * bestPhi) + perp * m;
        const vec2 uv0 = vec2(P0.x / aspect, P0.y);
    ...
    const float la = radians(uLightAngle);
    const float el = radians(uLightElev);
    const vec2 Lxy = vec2(cos(la), sin(la)) * cos(el);
    const float Lz = sin(el);
    const float Ls = dot(Lxy, dir);
    // half vector with the orthographic view direction (0,0,1)
    vec2 Hxy = Lxy;
    float Hz = Lz + 1.0;
    const float hLen = max(sqrt(dot(Hxy, Hxy) + Hz * Hz), 0.0001);
    Hxy /= hLen; Hz /= hLen;
    const float Hs = dot(Hxy, dir);

    const vec4 src = sampleTex(uvSrc);

    float shade;
    float spec = 0.0;
    bool backFace = false;
    if (useWrap) {
        const float sinPhi = sin(bestPhi);
        const float cosPhi = cos(bestPhi);
        ...
        const float ta = clamp(arcAvail / PI, 0.0, 1.0);
        const float active = ta * ta * (3.0 - 2.0 * ta);
        const float litFlat = uAmbient + (1.0 - uAmbient) * max(0.0, Lz);
        shade = mix(1.0, litFlat, active);
        // the raised tube shades the flat part behind the contact line:
        // cast a ray from the pixel toward the light, test the cylinder
        if (active > 0.0) {
            const float A = Ls * d - Lz * R;
            const float disc = A * A - d * d;
            if (disc > 0.0) {
                const float sd = sqrt(disc);
                if (sd > A) {             // a positive ray parameter exists
                    const float pen = clamp(0.2 * R, 0.004, 0.05);
                    const float tp = clamp(sd / pen, 0.0, 1.0);
                    const float occ = 1.0 - tp * tp * (3.0 - 2.0 * tp);
                    shade *= 1.0 - uShadow * active * occ;
                }
            }
        }
    }
